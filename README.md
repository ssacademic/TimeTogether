...
**`index.html` is the whole app.** It's fully self-contained — styling and all JavaScript are bundled directly into that one file, so there's nothing else to download or host alongside it, and no way for a missing file to break it. The other files (`style.css`, `data.js`, `core.js`, `app.js`) are the same code broken out for readability and testing — see "Project structure" below — but you never need them to actually run the app.

## What changed in this version
- **Fixed a real breakage**: an earlier version split the app across 5 files, and if `index.html` was opened without the other four sitting right next to it (e.g. only `index.html` got downloaded, or it was uploaded somewhere that didn't preserve the folder), the page loaded with an empty tabs bar, no rows, and no working buttons — everything JavaScript-driven silently failed. `index.html` is now fully self-contained, so this can't happen anymore.
- **Added a guided walkthrough**: a short, skippable 5-step tour explains groups, adding people, what the timeline row means, sorting, and the meeting-time suggester. Shown once on first visit (after the "who are you" step), replayable anytime via the "?" button in the top bar or Settings → Show walkthrough.
...
## Project structure
```
index.html   — the whole app, self-contained (open this — nothing else needed)
style.css    — same styling, as a standalone file for readability
...
```
`index.html` embeds the contents of `style.css`, `data.js`, `core.js`, and `app.js` directly — they're kept as separate files too only so the source is easier to read and so `core.js` can be unit-tested with plain Node without a browser. If you edit the standalone files, regenerate `index.html` by re-embedding them (or just edit the `<style>`/`<script>` blocks inside `index.html` directly — they're the same code).

## Run it locally
Open `index.html` in any browser — no build step, no install, no other files required.
...
2. Add just `index.html` to the repo root (the other files are optional — see "Project structure" above).
3. Commit and push.
